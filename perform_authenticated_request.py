...
if len(sys.argv) < 2:
    print("Provide full endpoint as only parameter")
    sys.exit()
...
service = 'execute-api'
host = ep.hostname
region = "eu-west-1" if os.getenv("AWS_DFAULT_REGION") is None else os.getenv("AWS_DFAULT_REGION")

# Alphabetical order of parameters is required to compute the request signature
params = ep.query.split("&")
params.sort()

request_parameters = ""
delim = ""
for s in params:
    request_parameters += delim + s
    if delim == "":
        delim = "&"
...
signed_headers = 'host;x-amz-date' + ';x-amz-security-token' if session_token is not None else None
